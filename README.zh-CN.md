...
- 权限确认双链路兜底：hook 生效时秒级提醒；hook 未生效时由会话记录启发式判定（编辑类约 30s / Bash 约 90s），卡住的会话不再静默
- 会话结束同样有通知（基于约 2 分钟静默判定）
- 按确认点去重（10 分钟），同一会话的多个确认各自提醒；通知开关在设置页

### ✅ 不切终端，直接批准

- 通过 Claude Code 官方 PreToolUse hook 机制，工具权限确认直接出现在面板里，点「允许 / 拒绝」即生效
- 不操作自动回落终端原生提示（等待时长可配 10–300 秒，默认 45 秒，hook 超时自动联动）；面板关着完全无感
- 安全闸门：面板窗口不可见直接放过（零延迟）、会话路径真实性校验、特殊权限模式放过、命中 allowlist 的命令放过——**任何异常只会回落终端，绝不缺省放行**
- 设置页一键安装/卸载 hook（写 `~/.claude/settings.json` 前自动备份）

### 🗂 会话管理

- 浏览、搜索**全部** Claude Code / Codex 历史会话（标题/项目搜索、按来源筛选、分页加载）
- 查看任意会话的完整对话内容，一键在终端恢复（`claude --resume` / `codex resume`）或打开项目目录
- 删除旧会话：两段式确认，移到废纸篓可恢复；进行中的会话由服务端兜底保护

### ⚙️ 独立设置页

- 主题：深色 / 浅色 / 跟随系统
- 语言：中文 / English（界面与系统通知同步切换）
- 开机自启、通知提示音开关、面板确认等待时长、恢复会话使用的终端（Terminal / iTerm2）
...
要启用「面板内批准」：打开面板 → 右上角 ⚙ 进入设置 → 勾选「在面板中确认工具权限」→ 新开的 Claude Code 会话生效。
